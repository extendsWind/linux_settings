// ==UserScript==
// @name        Baidu_Search_Result_Filter_remove_CSDN
// @author      dlee
// @version     1.0
// @update      2020-08-15
...
(function () {
    //黑名单, 新添加以"|"符号隔开即可
    //下面仅作为示例,不代表作者的任何观点
    const blankList = "csdn已为您找到|csdn.net|csdn技术社区";
    // const noBlankList = "csdn博客";
    const noBlankList = "SLDKFJSDLJF|aslsdfjkl";
    ...
    function removeitems() {
        ...
        let p = 0;
        for (i; i >= 0; i--) {
            // .getElementsByClassName("c-showurl")
            let urlElements = sDom[i].getElementsByClassName("c-showurl");
            if (urlElements.length <= 0){
                continue;
            }
            let stext = urlElements[0].innerText.toLowerCase();
          //  let stext = sDom[i].innerText.toLowerCase();
            console.log("-------------------------")
            console.log(stext)
            for (let k = 0; k < ilength; k++) {
                if (stext.includes(iTems[k]) === true) {
                    console.log(stext + " contained")
                    let mark0 = false;
